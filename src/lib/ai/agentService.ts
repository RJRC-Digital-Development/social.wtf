...
export class SocialWtfAiAgent {
  ...
  public async processUserPrompt(
    ...
    currentVerification: {
      ...
      isAdultContentUnlocked?: boolean;
    }
    ...
    if (
      ...
      const isCard = currentVerification.isCardVerified;
      const isVideo = currentVerification.isVideoVerified;
      const isId = currentVerification.isIdVerified;
      const under25 = currentVerification.isUnder25Flagged;
      const adultUnlocked = currentVerification.isAdultContentUnlocked;
    ...
    if (p.includes('tip') || p.includes('send cook') || p.includes('split') || p.includes('fee')) {
      ...
      return {
        ...
        sender: 'agent',
        timestamp,
        content: `**Cookie Chain 0.05% Protocol Fee Task Assistant:**\n\nEvery tip or storefront purchase is executed atomically on Cookie Chain SVM:\n- **Creator Direct:** 99.95% (${splitExample.creatorAmount} COOK for a 5 COOK tip)\n- **Social.wtf Treasury:** 0.05% (${splitExample.treasuryAmount} COOK to fund validator grants & platform innovation)\n\nWho would you like to tip?`,
        actions: [
          {
